...
bool InterArrival::ComputeDeltas(uint32_t timestamp,
  ...
  else if (!PacketInOrder(timestamp)) // TODO@chensong 2022-11-30 函数意义 ？？？ 为什么是uint32[0x80000000] 一半的大小 
  {
    return false;
  }
  else if (NewTimestampGroup(arrival_time_ms, timestamp)) 
  {
    ...
    if (prev_timestamp_group_.complete_time_ms >= 0) 
	{
      ...
      *packet_size_delta = static_cast<int>(current_timestamp_group_.size) - static_cast<int>(prev_timestamp_group_.size);
  ...
  {
    current_timestamp_group_.timestamp = LatestTimestamp(current_timestamp_group_.timestamp, timestamp);
  }
...
bool InterArrival::PacketInOrder(uint32_t timestamp) 
{
  ...
  else 
  {
	...
	/假设diff大于时间戳间隔的一半 （32位）必须是由于重新排序。此代码与
	*/
...
// TODO@chensong 2022-11-30 猜测该BelongsToBurst函数意义
[无线网情况下网络波动时算法优化 ？？？？？？？？？？？]
...
bool InterArrival::BelongsToBurst(int64_t arrival_time_ms, uint32_t timestamp) const 
{
  ...
  // TODO@chensong 2022-11-30  包组中第一个包到达时间和包组中最后到达时间的差值
  int64_t arrival_time_delta_ms = arrival_time_ms - current_timestamp_group_.complete_time_ms;
  // TODO@chensong 2022-11-30  包组中最后一个包的发送时间 与现在时间差
  uint32_t timestamp_diff = timestamp - current_timestamp_group_.timestamp;
  int64_t ts_delta_ms = timestamp_to_ms_coeff_ /*1.4901161193847656e-05*/ * timestamp_diff + 0.5;
  if (ts_delta_ms == 0)
  {
    return true;
  }
  int propagation_delta_ms = arrival_time_delta_ms - ts_delta_ms;
  if (propagation_delta_ms < 0 && arrival_time_delta_ms <= kBurstDeltaThresholdMs/*5*/ 
	  && arrival_time_ms - current_timestamp_group_.first_arrival_ms < kMaxBurstDurationMs/*100*/)
  {
    return true;
  }
  return false;
